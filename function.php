...
if($type1!='pdf' && $type1!='jpg' && $type1!='jpeg' && $type1!='png' && $type1!='gif' && $type1!='JPG' && $type1!='JPEG' && $type1!='PNG'  && $type1!='GIF' && $type1!='ico'){
 echo "Only pdf,JPG,JPEG,PNG and GIF file format are allowed to Upload";
 $success="0";
  return $success;
}else{
 ...
 $uplaod_success=move_uploaded_file($_FILES[$imagename]['tmp_name'],"".$target1);
    //   $uplaod_success=compressImage($_FILES[$imagename]['tmp_name'],$target1,60);
    if($uplaod_success)
    {
